...
export const genericNavItems: NavItem[] = [
...
export const GenericSidebar: React.FC<GenericSidebarProps> = ({
  ...
  const categories = Array.from(new Set(genericNavItems.map(item => item.category)));
    ...
    <>
      ...
      >
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {categories.map(cat => {
            const items = genericNavItems.filter(item => item.category === cat);
            return (
